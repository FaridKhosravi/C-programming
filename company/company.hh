...
// This is the header file of the company which I only add some functions and
//  one map(organization_) in the private part to help me in the programming
...
private:
    ...
    // I used this map to have access to all of the employees data easily
    // the key of the pair data is the id_ and the rest of the information is in Employee* struct.
    std::map<std::string, Employee*> organization_;

    // Finds the highest boss in employee's hierarchy and makes changes in the unconst inputs
    void highestBoss(const std::string& id, std::string &boss, std::map<std::string, double> &department) const;

    // recursive function for department method to find the all team member in a departement
    void departmentRec(const std::string& id, std::string boss, std::map<std::string, double> &department) const;

    // recursive function for shortest and longest methods
    void longestRec(const std::string& id, const std::string& id1, std::map<std::string, double> &department) const;

    // recursive function designed for printSubordinatesN
    void subordinatesNRec(const std::string &id, size_t it, int &level, const int n, std::vector<std::string> &submap) const;
    ...
    // Printing errors.
    void printNotFound(const std::string &id, const bool &bid, std::ostream &output) const;
